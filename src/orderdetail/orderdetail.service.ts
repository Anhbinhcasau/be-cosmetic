...
export class OrderdetailService {
  ...
  async changeStatusOrderDetail({ userId, orderDetailId, status }) {
    const foundOrderDetail = await this.orderDetailModel
      .findOne({ _id: orderDetailId, userId })
      .exec();
    if (!foundOrderDetail)
      throw new ForbiddenException('Lỗi không tìm thấy đơn hàng');

    const query = { _id: orderDetailId },
      updateSet = { status },
      option = { upsert: true };
  ...
  async isOrderDetailSuccess({ userId, productId }) {
    ...
    const foundProduct = foundOrderDetail
      .filter((value) => value.status === StatusOrderEnum.COMPLETED)
      .flatMap((val) => val.products)
